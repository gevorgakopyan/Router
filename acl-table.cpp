...
namespace simple_router
{
  ...
  {
    // ACLTableEntry *res_entry = new ACLTableEntry();
    // res_entry->priority = 0;
    ACLTableEntry res_entry;
    res_entry.priority = 1;

    // std::cerr << "ACL: " << std::endl;
    // std::cerr << (entry.src & entry.srcMask) << " " << (srcIp & entry.srcMask) << std::endl;
    // std::cerr << (entry.dest & entry.destMask) << " " << (srcPort & entry.srcPortMask) << std::endl;
    // std::cerr << (entry.srcPort & entry.srcPortMask) << " " << (srcPort & entry.srcPortMask) << std::endl;
    // std::cerr << (entry.destPort & entry.destPortMask) << " " << (dstPort & entry.destPortMask) << std::endl;
    // std::cerr << (entry.protocol & entry.protocolMask) << " " << (protocol & entry.protocolMask) << std::endl;

    for (auto entry : m_entries)
    {
      if (
          (entry.dest & entry.destMask) == (dstIp & entry.destMask) &&
          (entry.src & entry.srcMask) == (srcIp & entry.srcMask) &&
          (entry.srcPort & entry.srcPortMask) == (srcPort & entry.srcPortMask) &&
          (entry.destPort & entry.destPortMask) == (dstPort & entry.destPortMask) &&
          (entry.protocol & entry.protocolMask) == (protocol & entry.protocolMask) && (entry.priority > res_entry.priority))
      {
        // res_entry = &entry;
        res_entry.action = entry.action;
        res_entry.dest = entry.dest;
        res_entry.destMask = entry.destMask;
        res_entry.destPort = entry.destPort;
        res_entry.destPortMask = entry.destPortMask;
        res_entry.priority = entry.priority;
        res_entry.protocolMask = entry.protocolMask;
        res_entry.src = entry.src;
        res_entry.srcMask = entry.srcMask;
        res_entry.srcPort = entry.srcPort;
        res_entry.srcPortMask = entry.srcPortMask;
      }
    }

    if (res_entry.priority == 0)
    {
