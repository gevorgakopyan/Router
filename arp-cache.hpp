...
namespace simple_router
{
  ...
  struct ArpRequest
  {
    ArpRequest(uint32_t ip, std::string iface)
        : ip(ip), nTimesSent(0), iface(iface)
    {
    }
  ...
  public:
     ...
     */
    std::shared_ptr<ArpRequest> queueArpRequest(uint32_t ip, const Buffer &packet, const std::string &iface);
